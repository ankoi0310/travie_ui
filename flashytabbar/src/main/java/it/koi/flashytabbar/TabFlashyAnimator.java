...
public class TabFlashyAnimator extends ViewPager.SimpleOnPageChangeListener {
    ...
    private Integer color = Color.argb(255, 40, 45, 130);
    private int previousPosition = -1;

    public TabFlashyAnimator(TabLayout tabLayout, Integer color) {
    ...
    public void addTabItem(String title, Integer tabIcon, Integer color, Float size) {
        ...
        mFragmentColorList.add(color == null ? this.color : color);
        mFragmentSizeList.add(size);
    }

    private void getTabView(int position, TabLayout.Tab tab, boolean isSelected) {
        View view = tab.getCustomView() == null ? LayoutInflater.from(tabLayout.getContext()).inflate(R.layout.custom_tab, tabLayout, false) : tab.getCustomView();
        ...
        dot.setColorFilter(mFragmentColorList.get(position) == null ? Color.argb(255, 40, 45, 130) : getColor(mFragmentColorList.get(position)));
    ...
    public void setBadge(int count, int position) {
        ...
        assert tab != null;
        TextView badge = Objects.requireNonNull(tab.getCustomView()).findViewById(R.id.badge);
