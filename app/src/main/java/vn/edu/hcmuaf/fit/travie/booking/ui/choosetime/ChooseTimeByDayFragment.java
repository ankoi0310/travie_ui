...
import static vn.edu.hcmuaf.fit.travie.booking.ui.choosetime.ContinuousSelectionHelper.getSelection;
...
public class ChooseTimeByDayFragment extends Fragment {
    ...
    public static ChooseTimeByDayFragment newInstance(BookingType bookingType) {
    ...
    private void configureBinders() {
            ...
            public DayViewContainer(@NonNull View view) {
                ...
                view.setOnClickListener(v -> {
                    if (day.getPosition() == DayPosition.MonthDate &&
                        ...
                        if (selection.getEndDate() == null && day.getDate().isAfter(selection.getStartDate()) && day.getDate().minusDays(30).isAfter(selection.getStartDate())) {
                            Toast.makeText(context, "Chỉ chọn tối đa 30 ngày", Toast.LENGTH_SHORT).show();
                            return;
                        }

                        selection = getSelection(day.getDate(), selection);
                        binding.calendarView.notifyCalendarChanged();
                        bindSummaryViews();
                    }
                });
        ...
        binding.calendarView.setMonthHeaderBinder(new MonthHeaderFooterBinder<MonthViewContainer>() {
            ...
            public void bind(@NonNull MonthViewContainer container, CalendarMonth month) {
                String monthName = month.getYearMonth().getMonth().getDisplayName(TextStyle.FULL, Locale.getDefault());
                container.textView.setText(WordUtils.capitalize(monthName));
            }
        });
    }
}